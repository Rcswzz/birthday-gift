...
(() => {
  ...
  async function postToCloudBase(payload) {
    ...
    try {
      ...
      let token;
      try {
        const loginRes = await fetch(cfg.restBase + "/auth/v1/signin/anonymously", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "x-device-id": "web-" + Math.random().toString(36).slice(2, 10),
          },
          body: "{}",
      ...
      try {
        const res = await fetch(cfg.restBase + "/v1/rdb/rest/gift_choice?select=id", {
          method: "POST",
          headers: {
